...
async def hitecua(data, site):
    html = BeautifulSoup(data, 'html.parser')
    news = html.find('div', class_='main-content-single').find_all('article')
    News_list = []
    for item in news:
        title = item.find('div', class_='single-post-news-page').find('a').text
        short = item.find('div', class_='small-arial-text single-post-news-page-text').find('p').text
        origin = 'hi-tech.ua'
        url = item.find('div', class_='single-post-news-page').find('a').get('href')
        preview = item.find('div', class_='single-post-news-page-img').find('img').get('data-src')
        if preview == None:
            preview = item.find('div', class_='single-post-news-page-img').find('img').get('src')
        now = time.time()
        news_ = News_item(title, short, url, preview, now, origin)
        News_list.append(news_)
    manager = DbManager()
...
async def gamedevdou(data, site):
    html = BeautifulSoup(data, 'html.parser')
    news = html.find('div', 'b-lenta').find_all('article', class_='b-postcard')
    News_list = []
    for item in news:
        title = item.find('h2', class_='title').find('a').text.strip()
        short = item.find('p', class_='b-typo').text.strip()
        origin = 'gamedev.dou.ua'
        url = item.find('h2', class_='title').find('a').get('href')
        preview = item.find('img', 'announce-img').get('src')
        now = time.time()
        news_ = News_item(title, short, url, preview, now, origin)
        News_list.append(news_)
    manager = DbManager()
...
async def uaprogaming(data, site):
    html = BeautifulSoup(data, 'html.parser')
    news = html.find('div', id='blog-entries').find_all('article')
    News_list = []
    for item in news:
        title = item.find('h2', 'entry-title').find('a').text
        short = item.find('div', class_='entry-summary').find('p').text
        origin = 'uaprogaming.com.ua'
        url = item.find('h2', 'entry-title').find('a').get('href')
        preview = item.find('div', 'post-thumbnail').find('img').get('src')
        now = time.time()
        news_ = News_item(title, short, url, preview, now, origin)
        News_list.append(news_)
    manager = DbManager()
...
async def gagadget(data, site):
    ...
    for item in news:
        ...
        url = 'https://gagadget.com' + item.find('a', class_='b-indexnode__comments b-indexnode__comments_cell').get(
            'href')
        short = "None"
        origin = 'gagadget'
        now = time.time()
...
async def gamespot(data, site):
    ...
    allNews = soup.find_all("div",
                            class_="card-item base-flexbox flexbox-align-center width-100 border-bottom-grayscale--thin")
...
async def gameinformer(data, site):
    ...
    for article in allNews:
        try:
            ...
            short = article.find("div",
                                 class_='field field--name-field-promo-summary field--type-string field--label-hidden gi5-field-promo-summary gi5-string field__item').text
...
async def get_data(context):
    ...
    elif context['site'] == 'hi-tech.ua':
        await hitecua(context["data"], site=context['site'])
    elif context['site'] == 'DOU':
        await gamedevdou(context["data"], site=context['site'])
    elif context['site'] == 'uaprogaming':
        await uaprogaming(context["data"], site=context['site'])
...
async def main():
    async with aiohttp.ClientSession() as session:
        ...
        tasks = [
            ...
            {"site": "hi-tech.ua",           "url":"https://hi-tech.ua/uk/tag/igry-uk/"},
            {"site": "DOU",                  "url":"https://gamedev.dou.ua/news/"},
            {"site": "uaprogaming",          "url":"https://uaprogaming.com.ua/category/novyny/"}
        ]
